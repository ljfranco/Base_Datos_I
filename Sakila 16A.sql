...
columnas en español.*/
 ...
 select first_name as Nombre, last_name as Apellido, email as "Correo Electronico" from customer
 where active = 1;
...
/*2. Obtener la suma TOTAL de todos los pagos (payments). Utilizar un alias para mostrarlo en una columna llamada “total”,
 junto a una columna con la cantidad de alquileres con el alias “Cantidad” y una columna que 
indique el “Importe promedio” por alquiler. */
...
select cus.first_name as Nombre, cus.last_name as Apellido, sum(amount) as total, count(*) as cantidad from payment as pay
join customer as cus on cus. customer_id = pay.customer_id
...
select customer_id as "Id Cliente", sum(amount) as "monto total", count(*) as "cantidad Alquileres" from payment
...
select date_format(rental_date, "%M/%Y") as mes_año, sum(amount) as Total, count(*) as cantidad from rental
